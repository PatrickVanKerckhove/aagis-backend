...
type RequestValidationSchemeInput = 
  Partial<Record<'params' | 'body' | 'query', SchemaLike>>; // Partial = optioneel (?)
...
const validate = (scheme: RequestValidationSchemeInput | null) => {
  ...
  return (ctx: KoaContext, next: Next) => {
    ...
    const { 
      error: paramsErrors, 
      value: paramsValue, 
    } = parsedSchema.params.validate(
      ctx.params, JOI_OPTIONS);
